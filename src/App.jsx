...
async function extractVocab(apiKey, text) {
  if (!apiKey) return [];
  try {
    const result = await callAI(apiKey, VOCAB_EXTRACT_SYSTEM, [
      { role: "user", text: `學習內容：\n${text}` },
    ]);
...
export default function LearningEngine() {
  ...
  const startMethod = async (m) => {
    ...
    const initMsg = m.id === "feynman"
      ...
      : `我想開始學習「${topic}」，請出第一題。`;
    try {
      const aiText = await callAI(apiKey, SYSTEM_PROMPTS[m.id](topic), [{ role: "user", text: initMsg }]);
      setLoading(false);
  ...
  const sendMessage = async () => {
    ...
    setLoading(true);
    try {
      const aiText = await callAI(apiKey, SYSTEM_PROMPTS[method.id](topic), newMessages);
      setLoading(false);
      setTypingMsg({ role: "assistant", text: aiText, id: Date.now(), vocab: [] });
    } catch (e) {
      setLoading(false);
